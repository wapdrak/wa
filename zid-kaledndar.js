/**
 * ŽIDOVSKÝ KALENDÁŘ - ČISTÉ MULTI-LOKALITNÍ API
 * Generuje JSON bez sekce "dnes" – města jsou přímo v kořeni.
 *
 */

const fs = require('fs');

// 1. Lokality a jejich souřadnice
const LOKALITY = {
...
const vsechnyParashot = [
    ...
    "Vajikra (וַיִּקְרָא)", "Caw (צַו)", "Šmini (שְּׁמִינִי)", "Tazria (תַזְרִיעַ)", "Mecora (מְּצֹרָע)", "Acharej Mot (אַחֲרÉי מוֹת)", "Kedošim (קְדֹším)", "Emor (אֱמֹר)", "Behar (בְּהַר)", "Bechukotaj (בְּחֻקֹּתַי)",
    "Bemidbar (בְּmִדְבַּר)", "Naso (נָשֹׂא)", "Beha'alotecha (בְּהַעֲלֹתְךָ)", "Šlach Lecha (שְׁלַח-לְךָ)", "Korach (קֹרַח)", "Chukat (חֻקַּת)", "Balak (בָּלָק)", "Pinchas (פִּינְחָס)", "Matot (מַטּוֹת)", "Masej (מַסְעֵי)",
    "Devarim (דְּבָרִים)", "Va'etchanan (וָאֶתְחַנַּן)", "Ekev (עֵקֶב)", "Re'e (רְאÉ)", "Shoftim (שֹׁפְטִים)", "Ki tece (כִּי-תֵצֵא)", "Ki tavo (כִּי-תָבוֹא)", "Nicavim (נִצָּבִים)", "Vajelech (וַיֵּלֶךְ)", "Ha'azinu (הַאֲזִינוּ)", "Ve-zot ha-beracha (וְזֹאת הַבְּרָכָה)"
];
...
function getTzeitMin(date, lat, tz) {
...
function generate() {
    ...
    for (const [misto, conf] of Object.entries(LOKALITY)) {
        let d = new Date();
        if ((d.getHours() * 60 + d.getMinutes()) >= getTzeitMin(d, conf.lat, conf.tz)) d.setDate(d.getDate() + 1);
        ...
        // Vytvoření větve města bez sekce "dnes"
        apiVystup[misto] = {
            ...
            "svatek": svatek,
            "tzeit": Math.floor(getTzeitMin(new Date(), conf.lat, conf.tz) / 60) + ":" + (Math.floor(getTzeitMin(new Date(), conf.lat, conf.tz) % 60)).toString().padStart(2, '0')
        };
    }

    fs.writeFileSync('zid-kalendar.json', JSON.stringify(apiVystup, null, 2));
    console.log("JSON vygenerován se strukturou měst.");
}
